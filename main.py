...
def init_db():
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
        conn.close()
        # Auto-sync memory.json to database after tables are created
        sync_memory_to_db()
    except Exception as e:
...
def get_messages(user_id):
    ...
    if conn:
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT * FROM messages WHERE user_id = %s ORDER BY timestamp DESC LIMIT 50", (user_id,))
                msgs = [dict(row) for row in cur.fetchall()][::-1]
                if msgs:
                    logging.info(f"Retrieved {len(msgs)} messages from PostgreSQL for user {user_id}")
                    return msgs
        except Exception as e:
    ...
    try:
        ...
        if msgs:
            logging.info(f"Retrieved {len(msgs)} messages from TinyDB for user {user_id}")
...
def chat():
    ...
    recent_msgs = get_messages(OWNER_ID)[-10:]
    recent_context = "\n".join([f"U: {m['message']}\nB: {m['response']}" for m in recent_msgs])
    ...
    if is_admin:
        prompt = (f"{admin_instructions} You are speaking to your Creator/Admin. Be technical, obedient, and helpful. "
                  f"Current system state: {memory}\nCommand: {msg}")
    else:
        prompt = (f"{admin_instructions} It's {time_greeting}. Context: {memory}\n{recent_context}\nHer message: {msg}")
...
def admin_update_api():
    ...
    if data.get("database_url"): set_config('database_url', data["database_url"])
    return jsonify({"success": True})
